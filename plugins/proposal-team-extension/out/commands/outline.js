...
exports.handleOutline = handleOutline;
 ...
 */
async function handleOutline(request, stream, token, resources) {
...
Produce a numbered list of sections with their mapped requirements. Then list any orphans and empty sections.`;
    const framework = await (0, lmAgent_1.sendPrompt)(model, frameworkPrompt, token);
    ...
    const agentOutputs = await (0, lmAgent_1.runAgentsParallel)(model, agentPrompts, stream, token);
...
AGENT OUTPUTS:
${(0, lmAgent_1.formatAgentOutputs)(agentOutputs)}
...
Begin.`;
    ...
    const compiled = await (0, lmAgent_1.sendPromptStreaming)(model, compilePrompt, stream, token);
    // Write output files
...
function buildGrowthStrategistPrompt(agentDef, standards, phase1, context, skeleton) {
...
function buildComplianceReviewerPrompt(agentDef, phase1, skeleton) {
...
function buildSolutionArchitectPrompt(agentDef, standards, phase1, context, skeleton) {
...
function buildSMEPrompt(agentDef, standards, phase1, context, skeleton) {
