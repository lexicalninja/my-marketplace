 ...
 */
async function handleReview(request, stream, token, resources) {
...
Produce a concise Phase 1 summary covering all of the above.`;
    const phase1Summary = await (0, lmAgent_1.sendPrompt)(model, phase1Prompt, token);
    ...
    const debateResult = await (0, lmAgent_1.runDebate)(model, 3, // max rounds
    (round, previousOutputs) => {
        return agentNames.map((name) => ({
            ...
            prompt: round === 1
                ...
                : buildAlignmentFollowUpPrompt(agentDefs[name], formatAgentName(name), phase1Summary, previousOutputs || []),
        }));
    }, (outputs) => {
        // Simple convergence test: check if outputs mention agreement
        const agreedCount = outputs.filter((o) => o.text.toLowerCase().includes('agree') || o.text.toLowerCase().includes('consensus')).length;
        return agreedCount >= 3;
    }, stream, token);
    // Write Prop Manager determination
    const alignmentSummary = (0, lmAgent_1.formatAgentOutputs)(debateResult.rounds[debateResult.rounds.length - 1]);
    const phase2Determination = await (0, lmAgent_1.sendPrompt)(model, `You are the Prop Manager. Based on the following agent positions from the requirements alignment debate, produce a determination:
...
Write your determination in the debate template format.`, token);
    ...
    if (Object.keys(contextFiles).length === 0) {
        ...
        const contextContent = Object.entries(contextFiles)
            .map(([name, content]) => `### ${name}\n\n${content}`)
            .join('\n\n---\n\n');
        const contextAgents = agentNames.map((name) => ({
            name: formatAgentName(name),
...
- What's missing that you'd want to see`,
        ...
        const contextOutputs = await (0, lmAgent_1.runAgentsParallel)(model, contextAgents, stream, token);
        phase3Briefs = (0, lmAgent_1.formatAgentOutputs)(contextOutputs);
    ...
    for (const sectionFile of proposalSections) {
        ...
        const sectionAgents = agentNames.map((name) => ({
            name: formatAgentName(name),
            prompt: buildSectionAssessmentPrompt(agentDefs[name], formatAgentName(name), phase2Determination, phase3Briefs, scoreCardTemplate, writingStandards, sectionName, sectionContent),
        }));
        // Add Past Performance Specialist for PP sections
        if (isPastPerformance) {
            sectionAgents.push({
                name: 'Past Performance Specialist',
                prompt: buildPPAssessmentPrompt(ppDef, phase2Determination, phase3Briefs, scoreCardTemplate, sectionName, sectionContent),
            });
        }
        const sectionOutputs = await (0, lmAgent_1.runAgentsParallel)(model, sectionAgents, stream, token);
...
${Object.entries(allAssessments)
        .map(([section, outputs]) => `## ${section}\n\n${(0, lmAgent_1.formatAgentOutputs)(outputs)}`)
        .join('\n\n===\n\n')}
...
DEBATE TEMPLATE:
${debateTemplate}`;
    const reconciliation = await (0, lmAgent_1.sendPrompt)(model, reconciliationPrompt, token);
...
<!-- END ACTION_TRACKER -->`;
    ...
    const report = await (0, lmAgent_1.sendPromptStreaming)(model, reportPrompt, stream, token);
    // Write output files
...
function buildAlignmentPrompt(agentDef, agentName, phase1) {
...
function buildAlignmentFollowUpPrompt(agentDef, agentName, phase1, previousOutputs) {
...
function buildSectionAssessmentPrompt(agentDef, agentName, alignment, contextBriefs, scoreCardTemplate, writingStandards, sectionName, sectionContent) {
...
function buildPPAssessmentPrompt(ppDef, alignment, contextBriefs, scoreCardTemplate, sectionName, sectionContent) {
